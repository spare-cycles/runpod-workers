...
Imports are **inside** the branch on purpose. `transformers` and `librosa` are only present for the
challenger, and a top-level import of either would make a production worker — which loads neither —
pay their import cost on every cold start, and die outright if the image were ever slimmed down to
the production path alone.
"""

from __future__ import annotations

from ..config import QWEN3_ASR, VOXTRAL_SMALL, Config, ConfigError
